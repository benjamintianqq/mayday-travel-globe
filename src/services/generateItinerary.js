...
`.trim();
  ...
  if (part?.functionCall?.name === 'create_itinerary') {
    return part.functionCall.args;
  }

  // Fallback: try to find any functionCall
  const allParts = data.candidates?.[0]?.content?.parts ?? [];
  for (const p of allParts) {
    if (p.functionCall) return p.functionCall.args;
  }

  throw new Error('Gemini 未返回结构化行程数据，请重试');
}
